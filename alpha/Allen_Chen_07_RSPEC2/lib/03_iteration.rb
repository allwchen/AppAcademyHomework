...
def substrings(string)
  ...
  string.chars.each_with_index do |ch, idx|
    arr << ch
    (idx+1..str.length-idx).each_with_index do |num, idx2|
      substr = string[idx..idx+idx2]
      arr << substr
    end
  end
  return arr.uniq
...
def subwords(word, dictionary)
  ...
  dictionary.each do |entry|
    arr << entry if word.include?(entry)
  end
  return arr
end
...
def concatenate(strings)
  strings.inject{|str, word| str + word}
end
