...
describe "Code" do
  ...
  describe "PART 2" do
    ...
    describe "#num_near_matches" do
      ...
      #RGRB
      it "should return the number of pegs in the guess that are the correct color but incorrect position compared to @pegs" do
        expect(code.num_near_matches(Code.new(["G", "G", "G", "G"]))).to eq(0)
      end

      it "should return the number of pegs in the guess that are the correct color but incorrect position compared to @pegs" do
        expect(code.num_near_matches(Code.new(["R", "G", "G", "G"]))).to eq(0)
      end

      it "should return the number of pegs in the guess that are the correct color but incorrect position compared to @pegs" do
        expect(code.num_near_matches(Code.new(["R", "G", "G", "R"]))).to eq(1)
      end
